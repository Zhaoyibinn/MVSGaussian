...
def get_proj_mats(batch, src_scale, tar_scale):
    '''
    所谓的projs就是K * [R|t]最后再补全一行0001，4*4
    得到src相对于tar的坐标projs，参考视图相对于重建视图的projs
    '''
...
def build_feature_volume(feature, batch, D, depth, std, near_far, level):
    ...
    for s in range(S):
        ...
        # warped_volume:参考视角变换到当前视角的高维特征（变换 + 插值）
    ...
    # feature_volume : 最终的特征体是有均值和协方差cat组成的
    # depth_values : 很多层深度
    # near_far : 最近深度和最远深度
...
def sample_along_depth(rays, N_samples, level):
    '''
    沿深度采样，可以理解为深度其实是一个分布，然后取值，如果N_samples == 1 那么就取中电
    '''
...
def get_img_feat(xyz, img_feat_rgb, batch, training, level):# B * N * S * (11+4)
    ...
    for i in range(S):
        ...
        # 当前视图投影到三个参考视图的像素坐标，计算得到归一化的像素坐标
