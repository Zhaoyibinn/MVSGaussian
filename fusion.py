...
def fusion(dir_ply, scene):
    ...
    for name in img_name:
        ...
        try:
            ...
            ref_intrinsics, ref_extrinsics = read_camera_parameters(os.path.join(cam_dir, ref_name + '_cam.txt'))
        try:
            ref_img = read_img(os.path.join(img_dir, ref_name+'.png'))
        except:
            ref_img = read_img(os.path.join(img_dir, ref_name + '.jpg'))
        ...
        for src_name in src_names:
            print("fusing",src_name)
            # camera parameters of the source view
            try:
                src_intrinsics, src_extrinsics = read_camera_parameters(os.path.join(cam_dir, src_name+'.txt'))
            except:
                src_intrinsics, src_extrinsics = read_camera_parameters(os.path.join(cam_dir, src_name + '_cam.txt'))
